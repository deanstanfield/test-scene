...
Shader "Nuaj/ImageScalerLog"
{
	...
	SubShader
	{
		...
		// This pass downscales the main texture to a buffer at most 2x smaller and computes the log of the luminance
...
PARAM c[12] = { program.local[0..4],
		...
		{ 0.63999999, 0, 0.25, 9.9999997e-005 },
		{ 0.075300001, -0.2543, 1.1892, 0.69314718 },
...
def c8, -1.02170002, 1.97770000, 0.04390000, 0.00010000
def c9, 2.56509995, -1.16649997, -0.39860001, 0.69314718
...
def c5, -1.02170002, 1.97770000, 0.04390000, 0.00010000
def c6, 2.56509995, -1.16649997, -0.39860001, 0.69314718
...
PARAM c[2] = { program.local[0],
		{ 2.718282, 0.25, 0.75, 0.00390625 } };
TEMP R0;
TEMP R1;
TEMP RC;
TEMP HC;
OUTPUT oCol = result.color;
TEX   R1.x, c[1].zyzw, texture[0], 2D;
TEX   R0.x, c[1].y, texture[0], 2D;
MAXR  R0.y, R0.x, R1.x;
ADDR  R0.x, R0, R1;
TEX   R1.x, c[1].yzzw, texture[0], 2D;
ADDR  R0.z, R1.x, R0.x;
TEX   R0.x, c[1].z, texture[0], 2D;
ADDR  R0.z, R0.x, R0;
MAXR  R0.y, R0, R1.x;
MAXR  R0.x, R0.y, R0;
MULR  R0.z, R0, c[1].y;
POWR  R0.y, c[1].x, R0.z;
POWR  R0.x, c[1].x, R0.x;
ADDR  R0.x, R0, -R0.y;
MADR  R0.x, R0, c[0], R0.y;
FLRR  R0.y, R0.x;
MULR  R0.y, R0, c[1].w;
FLRR  R0.z, R0.y;
MULR  R0.z, R0, c[1].w;
...
"ps_3_0
dcl_2d s0
def c1, 0.25000000, 0.00000000, 0.75000000, 2.71828198
def c2, 0.00390625, 0, 0, 0
texldl r1.x, c1.zxzy, s0
texldl r0.x, c1.xxzy, s0
max r0.y, r0.x, r1.x
add r0.x, r0, r1
texldl r1.x, c1.xzzy, s0
add r0.z, r1.x, r0.x
texldl r0.x, c1.zzzy, s0
max r0.y, r0, r1.x
add r0.z, r0.x, r0
mul r1.x, r0.z, c1
max r2.x, r0.y, r0
